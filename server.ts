...
// On Cloud Run the platform assigns the port via PORT (8080 by default) and the
// container MUST listen there — a hardcoded 3000 fails the revision health check
// and the deploy dies with "failed to start and listen on the port". Seen live.
...
async function initializeApplication(app: express.Express) {
   ...
   * العميل بثابت الحزمة (__BUILD_ID__)، وتُكتب في dist/build-id.json عند البناء.
   */
  let cachedBuildId = '';
  ...
  if (!isProduction) {
    ...
    app.use(express.static(distPath, {
      ...
      setHeaders(res, filePath) {
        // الغلاف وعامل الخدمة وبصمة البناء لا تُخزَّن: نسخة قديمة منها تُجمّد التبويب
        // على إصدار سابق ولا يرى المتصفح تحديثاً أصلاً.
