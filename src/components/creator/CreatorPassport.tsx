...
export const CreatorPassport: React.FC<CreatorPassportProps> = ({ creatorId }) => {
  ...
  if (!profile) return <section className="glass-panel rounded-3xl p-6 text-center text-stone-500">ملف المبدع غير متاح لهذا المعرّف.</section>;

  return (
    <section className="glass-panel rounded-3xl border border-white/10 p-5 md:p-6 space-y-5">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
        <div className="flex items-center gap-4">
          <img src={profile.avatarUrl} alt={profile.displayName} decoding="async" className="w-16 h-16 rounded-2xl object-cover ring-2 ring-[#e8c880]/30" />
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-xl font-black text-stone-100">Creator Passport</h3>
              <span className="px-3 py-1 rounded-full bg-[#c7a55b]/10 text-[#e8c880] border border-[#e8c880]/20 text-[11px] font-black">{stage}</span>
            </div>
            <p className="text-xs text-stone-400 mt-1">جواز تجاري حي يلخص ما أثبته المبدع فعليًا داخل «مجال».</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Keep It', value: `${stats.keepRate}%`, icon: <Repeat2 className="w-4 h-4 text-[#e8c880]" /> }
        ].map((item, idx) => (
          <div key={idx} className="rounded-2xl p-4 bg-white/5 border border-white/10">
            <div className="flex items-center gap-2 text-[11px] text-stone-400">{item.icon}{item.label}</div>
            <div className="mt-2 text-xl font-black text-stone-100 font-mono">{item.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_1fr] gap-4">
        <div className="rounded-2xl p-4 bg-white/5 border border-white/10 space-y-3">
          <div className="flex items-center gap-2 font-bold text-stone-100"><Gauge className="w-4 h-4 text-sky-300" /> مؤشرات الثقة المهنية</div>
          {[
            ...
            <div key={idx}>
              <div className="flex justify-between text-xs mb-1"><span className="text-stone-400">{label as string}</span><strong className="text-stone-100">{value as number}%</strong></div>
              <div className="h-2 rounded-full bg-slate-950/70 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-l from-[#c7a55b] to-emerald-400" style={{ width: `${value}%` }} /></div>
            </div>
          ))}
        </div>

        <div className="rounded-2xl p-4 bg-white/5 border border-white/10 space-y-3">
          <div className="flex items-center gap-2 font-bold text-stone-100"><Crown className="w-4 h-4 text-[#e8c880]" /> Graduation Path</div>
          {[
            ...
            <div key={idx} className={`flex items-center justify-between p-3 rounded-xl border ${step.done ? 'bg-emerald-500/8 border-emerald-400/15' : 'bg-white/[0.02] border-white/10'}`}>
              <div className="flex items-center gap-2 text-xs font-bold text-stone-200">{step.icon}{step.label}</div>
              <span className={`text-[10px] font-black ${step.done ? 'text-emerald-300' : 'text-stone-500'}`}>{step.done ? 'مكتمل' : 'قادم'}</span>
            </div>
          ))}
        </div>
      </div>
