  ...
  return (
    <section className="glass-panel rounded-3xl border border-white/10 p-5 md:p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-black text-stone-100">Recipe Access Center — أذونات الوصفات</h3>
          <p className="text-xs text-stone-400 mt-1">أنت من يقرر من يرى ماذا، ولأي مدة، وبأي مستوى إفصاح.</p>
        </div>
      </div>
            ...
            <div key={grant.id} className="rounded-2xl p-4 bg-white/5 border border-white/10 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-black text-stone-100">{product?.publicName}</span>
                  <span className={`px-2 py-1 rounded-full text-[10px] font-black ${grant.status === 'REQUESTED' ? 'bg-amber-500/10 text-amber-300 border border-amber-400/20' : 'bg-emerald-500/10 text-emerald-300 border border-emerald-400/20'}`}>
                    {grant.status === 'REQUESTED' ? 'طلب جديد' : 'إذن فعّال'}
                  </span>
                </div>
                <div className="text-xs text-stone-400 mt-2 leading-6">{host?.commercialName} — مستوى الإفصاح المطلوب/المعتمد: <strong className="text-stone-200">L{grant.disclosureLevel}</strong></div>
                <div className="text-[11px] text-stone-500 mt-1">السبب: {grant.purpose}</div>
              </div>
              ...
              {grant.status === 'REQUESTED' ? (
                <div className="flex flex-wrap gap-2">
                  ...
                  <button onClick={async () => { await Promise.resolve(store.approveRecipeAccess(grant.id, grant.disclosureLevel)); }} className="px-3 py-2 rounded-xl bg-emerald-500 text-stone-950 text-xs font-black flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4" /> اعتماد المطلوب</button>
                  <button onClick={async () => { await Promise.resolve(store.revokeRecipeAccess(grant.id)); }} className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-stone-300 text-xs font-black flex items-center gap-1.5"><XCircle className="w-4 h-4" /> رفض</button>
                </div>
              ) : (
