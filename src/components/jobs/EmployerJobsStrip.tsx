...
export const EmployerJobsStrip: React.FC<{ onCreate: () => void }> = ({ onCreate }) => {
  ...
  return <section className="glass-card rounded-2xl border border-emerald-400/15 overflow-hidden">
    <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <button type="button" onClick={() => setExpanded(v => !v)} className="text-start flex items-center gap-3 min-w-0 flex-1">
        ...
        {jobs.length > 0 && <ChevronLeft className={`w-4 h-4 text-slate-400 ms-auto transition ${expanded ? '-rotate-90' : ''}`} />}
      </button>
