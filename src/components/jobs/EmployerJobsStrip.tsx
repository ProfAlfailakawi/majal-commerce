  ...
  return <section className="glass-card rounded-2xl border border-emerald-400/15 overflow-hidden">
    <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <button type="button" onClick={() => setExpanded(v => !v)} className="text-right flex items-center gap-3 min-w-0 flex-1">
        ...
        {jobs.length > 0 && <ChevronLeft className={`w-4 h-4 text-slate-400 mr-auto transition ${expanded ? '-rotate-90' : ''}`} />}
      </button>
    ...
    {expanded && jobs.length > 0 && <div className="border-t border-white/5 px-5 pb-5 pt-4 space-y-2">
      {jobs.slice(0, 8).map(job => <div key={job.id} className="rounded-xl p-3 bg-white/[0.025] border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div className="min-w-0"><div className="font-black text-xs text-slate-100 truncate">{job.title}</div><div className="text-[11px] text-slate-400 mt-1 flex items-center gap-1"><Clock3 className="w-3 h-3" />{statusLabel(job.status)} · {job.location}</div>{job.adminNote && <div className="text-[11px] text-amber-300 mt-1">ملاحظة الإدارة: {job.adminNote}</div>}</div><JobApplicationsButton job={job} /></div>)}
    </div>}
  </section>;
};
