    ...
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="job-composer-title" className="w-full max-w-2xl max-h-[90dvh] overflow-y-auto glass-panel rounded-[28px] border border-white/10 shadow-2xl">
      ...
      <form onSubmit={submit} className="p-5 space-y-4">
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <label className="sm:col-span-2 text-xs font-bold text-slate-300">المتطلبات <span className="text-slate-500 font-medium">(كل شرط في سطر)</span><textarea value={requirements} onChange={e=>setRequirements(e.target.value)} rows={3} className="mt-1.5 w-full glass-input rounded-xl px-4 py-3 text-sm outline-none resize-none" placeholder={'خبرة في التشغيل\nإجادة التعامل مع فرق العمل'} /></label>
          <label className="text-xs font-bold text-slate-300">راتب من <span className="text-slate-500">د.ك — اختياري</span><input value={salaryMin} onChange={e=>setSalaryMin(e.target.value)} inputMode="decimal" className="mt-1.5 w-full glass-input rounded-xl px-4 py-3 text-sm outline-none" /></label>
          <label className="text-xs font-bold text-slate-300">راتب إلى <span className="text-slate-500">د.ك — اختياري</span><input value={salaryMax} onChange={e=>setSalaryMax(e.target.value)} inputMode="decimal" className="mt-1.5 w-full glass-input rounded-xl px-4 py-3 text-sm outline-none" /></label>
        </div>
