...
export const JobApplicationsButton: React.FC<{ job: KuwaitiJobPost }> = ({ job }) => {
  ...
  return <>
    <button type="button" onClick={() => void show()} className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs font-black text-slate-200 hover:border-sky-400/20 transition inline-flex items-center gap-1.5"><UserRound className="w-3.5 h-3.5" />طلبات التوظيف</button>
      ...
      <div role="dialog" aria-modal="true" aria-label={`طلبات ${job.title}`} className="w-full max-w-2xl max-h-[86vh] overflow-hidden glass-panel rounded-3xl border border-white/10 shadow-2xl">
        ...
        <div className="p-5 overflow-y-auto max-h-[68vh] space-y-3">
          ...
          {!loading && applications.map(app => <article key={app.id} className="rounded-2xl p-4 bg-white/[0.025] border border-white/10 space-y-3">
            <div className="flex items-center justify-between gap-3"><div><h4 className="font-black text-sm text-slate-100">{app.fullName}</h4><p className="text-xs text-slate-400 mt-1">خبرة {app.yearsExperience} سنة · إقرار كويتي مقدم</p></div><span className="text-xs text-slate-400">{new Date(app.createdAt).toLocaleDateString('ar-KW')}</span></div>
            {app.summary && <p className="text-xs text-slate-400 leading-6">{app.summary}</p>}
            <div className="flex flex-wrap gap-2"><a href={`mailto:${app.email}`} className="px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-300 inline-flex items-center gap-1"><Mail className="w-3 h-3" />{app.email}</a><a href={`tel:${app.phone}`} className="px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-300 inline-flex items-center gap-1"><Phone className="w-3 h-3" />{app.phone}</a></div>
          </article>)}
          {!loading && applications.length > 0 && <p className="text-xs text-slate-400 leading-5">إقرار المتقدم بأنه كويتي ليس بديلاً عن التحقق الرسمي من الجنسية؛ أي تحقق حكومي يحتاج تكاملًا معتمدًا.</p>}
        </div>
      </div>
    </div>}
  </>;
