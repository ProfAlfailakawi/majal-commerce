  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      <section className="glass-panel rounded-[34px] p-6 md:p-10 border border-white/10 relative overflow-hidden">
        <div className="absolute -top-32 left-0 w-96 h-96 rounded-full bg-[#c7a55b]/10 blur-3xl" />
        <div className="relative z-10 grid lg:grid-cols-[1.05fr_.95fr] gap-8 items-center">
          <div className="space-y-5">
            ...
            <h1 className="text-3xl md:text-5xl font-black leading-tight">مو مجرد طلب أكل.<br /><span className="text-[#e8c880]">أنت تشارك في اكتشاف الاسم القادم.</span></h1>
            <p className="text-sm text-stone-400 max-w-2xl leading-7">كل منتج هنا مرّ بمبدع، منشأة مرخّصة، اختبار، اتفاق، وإطلاق. جرّبه، قيّمه، وقرر مع الجمهور هل يستحق البقاء.</p>

            {featuredCreator && (
              <div className="flex items-center gap-3 rounded-2xl p-3 bg-white/5 border border-white/10 w-fit">
                <img src={featuredCreator.avatarUrl} alt={featuredCreator.displayName} decoding="async" className="w-11 h-11 rounded-xl object-cover" />
                <div><div className="text-xs font-black text-stone-100">{featuredCreator.displayName}</div><div className="text-[10px] text-stone-500 mt-1">{featuredCreator.specialty}</div></div>
                <button
                  ...
                  className={`mr-3 px-3 py-2 rounded-xl text-[10px] font-black flex items-center gap-1.5 ${followedCreators.includes(featuredCreator.id) ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-400/20' : 'bg-[#c7a55b] text-stone-950'}`}
                >
          ...
          {featured && featuredProduct && (
            <div className="rounded-[28px] overflow-hidden bg-slate-950/50 border border-white/10 shadow-2xl">
              <div className="relative h-72">
                ...
                <div className="absolute top-4 right-4 px-3 py-1.5 rounded-full bg-slate-950/75 backdrop-blur text-[#e8c880] border border-[#e8c880]/20 text-xs font-black">{featured.sellingPriceKwd.toFixed(3)} د.ك</div>
              ...
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-4 gap-2 text-center">
                  <div className="rounded-xl p-3 bg-white/5"><Star className="w-4 h-4 text-[#e8c880] mx-auto" /><div className="text-xs font-black mt-1">{metrics.rating.toFixed(1)}</div><div className="text-[9px] text-stone-500">الطعم</div></div>
                  <div className="rounded-xl p-3 bg-white/5"><Repeat2 className="w-4 h-4 text-emerald-300 mx-auto" /><div className="text-xs font-black mt-1">{metrics.repeat}%</div><div className="text-[9px] text-stone-500">يكرر</div></div>
                  <div className="rounded-xl p-3 bg-white/5"><Users className="w-4 h-4 text-sky-300 mx-auto" /><div className="text-xs font-black mt-1">{metrics.keep}%</div><div className="text-[9px] text-stone-500">Keep It</div></div>
                  <div className="rounded-xl p-3 bg-white/5"><PackageOpen className="w-4 h-4 text-rose-300 mx-auto" /><div className="text-xs font-black mt-1">{metrics.remaining}</div><div className="text-[9px] text-stone-500">متبقي</div></div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => { setSelectedLaunch(featured); setSelectedBranchId(featured.branches[0] || ''); }} className="flex-1 py-3 rounded-xl bg-[#c7a55b] text-stone-950 text-xs font-black flex items-center justify-center gap-2"><ShoppingBag className="w-4 h-4" /> اطلب التجربة</button>
          ...
          {!featured && (
            <div className="rounded-[28px] min-h-72 bg-slate-950/45 border border-dashed border-white/15 grid place-items-center p-8 text-center">
              <div className="space-y-4 max-w-sm"><div className="w-16 h-16 mx-auto rounded-3xl bg-[#c7a55b]/10 border border-[#e8c880]/20 grid place-items-center"><PackageOpen className="w-8 h-8 text-[#e8c880]" /></div><h2 className="text-xl font-black">الإطلاق القادم يُجهّز الآن</h2><p className="text-xs text-stone-400 leading-6">لا نعرض منتجات غير مكتملة. يظهر أول Drop هنا بعد اجتياز الاختبار والامتثال وبوابة الإطلاق.</p></div>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="space-y-5">
        <div className="flex items-center justify-between gap-4"><div><h2 className="text-2xl font-black">الإطلاقات الحالية</h2><p className="text-xs text-stone-500 mt-1">منتجات محدودة، تجريبية، موسمية أو مرشحة للدخول الدائم.</p></div><div className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-stone-400">{launches.length} إطلاق</div></div>
            ...
            return (
              <article key={launch.id} className="glass-card rounded-3xl border border-white/10 overflow-hidden hover:-translate-y-1 transition-transform">
                <div className="relative h-52">
                  ...
                  <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-slate-950/75 border border-white/10 text-[10px] text-[#e8c880] font-black">{launch.launchType}</div>
                ...
                <div className="p-5 space-y-4">
                  <p className="text-xs text-stone-400 leading-6 line-clamp-2">{product?.shortDescription}</p>
                  <div><div className="flex justify-between text-[10px] text-stone-500 mb-1"><span>{launch.unitsSold} مبيعة</span><span>{progress}%</span></div><div className="h-2 bg-white/5 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-l from-[#c7a55b] to-emerald-400 rounded-full" style={{ width: `${progress}%` }} /></div></div>
                  <div className="flex items-center justify-between"><div><div className="text-[10px] text-stone-500">السعر</div><div className="font-black text-[#e8c880]">{launch.sellingPriceKwd.toFixed(3)} د.ك</div></div><button onClick={() => { setSelectedLaunch(launch); setSelectedBranchId(launch.branches[0] || ''); }} className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-xs font-black text-stone-100">شاهد واطلب</button></div>
                </div>
              </article>
            );
          })}
          {!launches.length && <div className="md:col-span-2 lg:col-span-3 rounded-3xl p-8 border border-dashed border-white/15 bg-white/[0.02] text-center"><Sparkles className="w-7 h-7 text-[#e8c880] mx-auto" /><div className="font-black mt-3">لا توجد إطلاقات متاحة حاليًا</div><div className="text-xs text-stone-500 mt-2">لن يظهر زر الطلب قبل اكتمال الجاهزية وربط الدفع.</div></div>}
        </div>
      </section>
      ...
      <section className="grid md:grid-cols-3 gap-4">
        {[
          ...
          { icon: <TrendingUp className="w-5 h-5 text-[#e8c880]" />, title: 'صوتك له قيمة', text: 'التقييم وKeep It يساعدان في قرار استمرار المنتج فعليًا.' }
        ].map((item, idx) => <div key={idx} className="glass-card rounded-2xl p-5 border border-white/10"><div>{item.icon}</div><h3 className="font-black mt-4">{item.title}</h3><p className="text-xs text-stone-400 leading-6 mt-2">{item.text}</p></div>)}
      </section>
      ...
      {selectedLaunch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md">
          <div ref={orderDialogRef} role="dialog" aria-modal="true" aria-labelledby="consumer-order-title" className="glass-panel w-full max-w-lg rounded-[28px] p-6 border border-white/15 shadow-2xl space-y-5 relative">
            <button aria-label="إغلاق نافذة الطلب" onClick={closeOrderModal} className="absolute top-4 left-4 p-2 rounded-full bg-white/5 text-stone-400 hover:text-white"><X className="w-4 h-4" /></button>
            <div><span className="text-[10px] text-[#e8c880] font-black">MAJAL DROP</span><h3 id="consumer-order-title" className="text-xl font-black mt-1">{selectedLaunch.title}</h3></div>

            {orderSuccessMsg ? (
              <div role="status" className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-400/20 text-center space-y-3"><CheckCircle2 className="w-10 h-10 text-emerald-300 mx-auto" /><div className="font-black text-emerald-300">{orderSuccessMsg}</div><p className="text-xs text-stone-400">لم تُخصم أي أموال، ولم تُحتسب مبيعات أو مستحقات.</p></div>
            ) : (
              <form onSubmit={handleOrder} className="space-y-4">
                ...
                <label className="block"><span className="text-xs text-stone-400 block mb-1.5">الاسم</span><input value={customerName} onChange={e => setCustomerName(e.target.value)} maxLength={80} className="w-full glass-input rounded-xl px-4 py-3 text-xs outline-none" required /></label>
                <label className="block"><span className="text-xs text-stone-400 block mb-1.5">رقم الهاتف</span><input value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} inputMode="tel" maxLength={16} className="w-full glass-input rounded-xl px-4 py-3 text-xs outline-none" required /></label>
                {selectedLaunch.branches.length > 0 && (
                  <label className="block">
                    <span className="text-xs text-stone-400 block mb-1.5">الفرع المرتبط بالطلب التجريبي</span>
