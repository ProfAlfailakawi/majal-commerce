  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-stone-100">
      <section className="glass-panel rounded-[30px] p-6 md:p-8 border border-white/10 relative overflow-hidden">
        <div className="absolute -top-24 -right-20 w-72 h-72 rounded-full bg-sky-400/8 blur-3xl" />
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="px-2.5 py-1 rounded-full bg-[#c7a55b]/10 text-[#e8c880] border border-[#e8c880]/20 text-[10px] font-black">{roleLabel(store.activeUser.role)}</span>
              </div>
              <h1 className="text-2xl md:text-3xl font-black mt-2">{host.commercialName}</h1>
              <p className="text-sm text-stone-400 mt-2 max-w-2xl leading-7">Innovation OS للمنشأة: اكتشاف مواهب، اختبار منتج، تفاوض، إطلاق، تشغيل وقياس — بصلاحيات مختلفة لكل عضو فريق.</p>
            </div>
          </div>

          {hasPermission(store.activeUser, 'MANAGE_CHALLENGES') && (
            <button onClick={() => setShowPublisher(true)} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-[#c7a55b] to-[#e1c67b] text-stone-950 text-xs font-black flex items-center gap-2"><Plus className="w-4 h-4" /> نشر تحدي ابتكار</button>
          )}
        </div>
      </section>
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar border-b border-white/10 pb-3">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2.5 rounded-xl whitespace-nowrap text-xs font-bold flex items-center gap-2 ${activeTab === tab.id ? 'bg-sky-400 text-slate-950' : 'bg-white/5 text-stone-300 hover:bg-white/10'}`}>
            {tab.icon}{tab.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              { label: canSeeFinance ? 'صافي المنشأة' : 'المنتجات النشطة', value: canSeeFinance ? `${finance.hostNet.toFixed(3)} د.ك` : store.products.filter(p => myCollaborations.some(c => c.productId === p.id) && !['PAUSED','COMPLETED'].includes(p.status)).length, icon: <CircleDollarSign className="w-4 h-4 text-[#e8c880]" /> }
            ].map((item, idx) => (
              <div key={idx} className="glass-card rounded-2xl p-4 border border-white/10"><div className="flex items-center gap-2 text-[11px] text-stone-400">{item.icon}{item.label}</div><div className="mt-2 text-xl font-black text-stone-100 font-mono">{item.value}</div></div>
            ))}
          </div>
          ...
          {!canDiscover && !canSeeTwin && !canSeeWarRoom && canSeeFinance && (
            <div className="glass-panel rounded-3xl p-6 border border-white/10 text-sm text-stone-400 leading-7">هذا الحساب مالي؛ تم إخفاء أدوات الاكتشاف والوصفة والمختبر. استخدم تبويب المالية لمتابعة الأرقام المصرح بها فقط.</div>
          )}
        </div>
      )}
      ...
      {activeTab === 'CHALLENGES' && (
        <div className="space-y-5">
          <div className="glass-panel rounded-3xl p-5 border border-white/10 flex items-center justify-between gap-4">
            <div><h3 className="font-black text-lg">تحديات الابتكار</h3><p className="text-xs text-stone-400 mt-1">حوّل احتياج المنشأة إلى Brief واضح يستقبل حلول المبدعين.</p></div>
            <button onClick={() => setShowPublisher(true)} className="px-4 py-2.5 rounded-xl bg-[#c7a55b] text-stone-950 text-xs font-black flex items-center gap-2"><Plus className="w-4 h-4" /> تحدٍ جديد</button>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            {store.challenges.filter(c => c.hostBusinessId === currentHostId).map(ch => (
              <article key={ch.id} className="glass-card rounded-2xl p-5 border border-white/10 space-y-3">
                <div className="flex items-center justify-between"><span className="px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-400/15 text-[10px] font-black">{ch.status}</span><span className="text-[10px] text-stone-500">{ch.estimatedVolumeUnits} وحدة متوقعة</span></div>
                <h4 className="font-black text-stone-100">{ch.title}</h4>
                <p className="text-xs text-stone-400 leading-6">{ch.brief}</p>
      ...
      {activeTab === 'FINANCE' && (
        <section className="glass-panel rounded-3xl p-5 md:p-6 border border-white/10 space-y-5">
          <div className="flex items-center gap-3"><div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-400/20 flex items-center justify-center text-emerald-300"><CircleDollarSign className="w-6 h-6" /></div><div><h3 className="text-lg font-black">Finance Surface</h3><p className="text-xs text-stone-400 mt-1">هذه الشاشة تظهر فقط للأدوار المصرح لها ماليًا.</p></div></div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ['GMV', finance.gmv], ['صافي المنشأة', finance.hostNet], ['حقوق المبدعين', finance.creatorRoyalties], ['رسوم مجال', finance.platformFees]
            ].map(([label, value], idx) => <div key={idx} className="rounded-2xl p-4 bg-white/5 border border-white/10"><div className="text-xs text-stone-400">{label as string}</div><div className="mt-2 text-xl font-black font-mono">{Number(value).toFixed(3)} د.ك</div></div>)}
          </div>
          <div className="rounded-2xl p-4 bg-sky-500/5 border border-sky-400/15 text-xs text-stone-300 leading-6 flex gap-2"><ShieldCheck className="w-4 h-4 shrink-0 mt-1 text-sky-300" /> فصل الصلاحيات المالية يمنع الشيف والتسويق من تعديل أو اعتماد قواعد التسوية والعقود المالية.</div>
        </section>
      )}
