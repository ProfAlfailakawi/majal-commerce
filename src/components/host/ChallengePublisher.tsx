...
export const ChallengePublisher: React.FC<ChallengePublisherProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="challenge-publisher-title" className="glass-panel border border-white/10 rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl text-slate-100 flex flex-col max-h-[92dvh]">
        <div className="p-5 bg-slate-950/45 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-gold-300/10 text-gold-300 rounded-xl border border-gold-300/20"><Briefcase className="w-5 h-5" /></div>
            <div>
              <h3 id="challenge-publisher-title" className="font-black text-base">نشر تحدي ابتكار جديد</h3>
              <p className="text-xs text-slate-400 mt-1">المنشأة: {host?.commercialName || 'غير محددة'}</p>
            </div>
          </div>
          <button onClick={onClose} aria-label="إغلاق ناشر التحدي" className="p-2 text-slate-400 hover:text-slate-100 rounded-xl hover:bg-white/5"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-xs">
          ...
          <label className="block">
            <span className="block text-slate-300 font-bold mb-1.5">عنوان التحدي</span>
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="مثال: حلوى كويتية جديدة لموسم الشتاء" className="w-full glass-input rounded-xl p-3 text-slate-100 outline-none" />
          </label>
          ...
          <div className="grid sm:grid-cols-3 gap-4 rounded-2xl p-4 bg-slate-950/35 border border-white/10">
            <label><span className="block text-slate-400 font-bold mb-1.5">سعر البيع المستهدف</span><input type="number" min="0.1" step="0.25" value={targetPriceKwd} onChange={e => setTargetPriceKwd(Number(e.target.value))} className="w-full glass-input rounded-xl p-2.5 text-gold-300 font-bold outline-none" /></label>
            <label><span className="block text-slate-400 font-bold mb-1.5">سقف التكلفة</span><input type="number" min="0" step="0.25" value={costCeilingKwd} onChange={e => setCostCeilingKwd(Number(e.target.value))} className="w-full glass-input rounded-xl p-2.5 outline-none" /></label>
