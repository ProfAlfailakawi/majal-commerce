...
const roleIcon = (role: string) => {
  if (role === 'HOST_OWNER') return <Crown className="w-4 h-4 text-[#e8c880]" />;
  ...
  return (
    <section className="glass-panel rounded-3xl border border-white/10 p-5 md:p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-black text-stone-100">Team & Permissions — فريق المنشأة</h3>
          <p className="text-xs text-stone-400 mt-1">لا يرى كل موظف كل شيء. الصلاحيات تتبع الدور، والسياق، وحساسية البيانات.</p>
        </div>
      </div>
        ...
        {team.map(member => {
          ...
          return (
            <div key={member.id} className="rounded-2xl p-4 bg-white/5 border border-white/10 space-y-3">
              <div className="flex items-center gap-3">
                <img src={member.avatar} alt={member.name} loading="lazy" decoding="async" className="w-11 h-11 rounded-xl object-cover" />
                <div className="min-w-0">
                  <div className="font-black text-stone-100 truncate">{member.name}</div>
                  <div className="text-xs text-stone-400 mt-1 flex items-center gap-1.5">{roleIcon(member.role)} {roleLabel(member.role)}</div>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {permissions.slice(0, 7).map(permission => (
                  <span key={permission} className="px-2 py-1 rounded-lg bg-slate-950/50 border border-white/10 text-[10px] text-stone-400">{permission}</span>
                ))}
              </div>
              <div className="rounded-xl p-3 bg-emerald-500/5 border border-emerald-400/15 text-[11px] text-stone-400 leading-6 flex gap-2"><ShieldCheck className="w-4 h-4 shrink-0 text-emerald-300" /> صلاحيات هذا العضو لا تتجاوز نطاق منشأته.</div>
            </div>
          );
        })}
      </div>
