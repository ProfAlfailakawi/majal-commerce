  ...
  return (
    <section className="glass-panel rounded-3xl border border-white/10 overflow-hidden">
      <button
        ...
        className="w-full p-5 flex items-center justify-between gap-4 text-start hover:bg-white/[0.025] transition"
              ...
              <article key={supplier.id} className="rounded-2xl p-4 bg-white/[0.025] border border-white/10 space-y-3">
                ...
                {supplierOfferings.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {supplierOfferings.map(item => <span key={item.id} className="px-2.5 py-1 rounded-lg bg-sky-500/8 border border-sky-400/10 text-xs text-sky-200 inline-flex items-center gap-1"><Boxes className="w-3 h-3" />{item.name}</span>)}
                  </div>
                )}
              </article>
            );
