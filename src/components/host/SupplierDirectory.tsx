            ...
            return (
              <article key={supplier.id} className="rounded-2xl p-4 bg-white/[0.025] border border-white/10 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="shrink-0 inline-flex items-center gap-1 text-[10px] font-bold text-emerald-300"><PackageCheck className="w-3.5 h-3.5" /> معتمد</span>
                </div>
                ...
                {supplierOfferings.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {supplierOfferings.map(item => <span key={item.id} className="px-2.5 py-1 rounded-lg bg-sky-500/8 border border-sky-400/10 text-[10px] text-sky-200 inline-flex items-center gap-1"><Boxes className="w-3 h-3" />{item.name}</span>)}
                  </div>
                )}
              </article>
            );
