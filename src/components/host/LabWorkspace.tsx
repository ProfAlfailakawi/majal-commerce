...
export const LabWorkspace: React.FC<LabWorkspaceProps> = ({ collaboration }) => {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      ...
      <section className="glass-panel p-6 rounded-3xl border border-white/10 space-y-4">
        <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><History className="w-4 h-4 text-sky-300" /><h4 className="font-black">سجل الدفعات</h4></div><span className="text-xs text-slate-400">{batches.length} دفعات</span></div>
        ...
        /> : batches.map(b => <div key={b.id} className="p-4 rounded-2xl bg-slate-950/40 border border-white/10 space-y-3"><div className="flex items-center justify-between gap-3 flex-wrap"><span className="font-black text-gold-300">{new Date(b.batchDate).toLocaleString('ar-KW')} — {b.recipeVersion}</span><span className={`px-2.5 py-1 rounded-full text-[11px] font-black border ${b.decision === 'PRODUCTION_CANDIDATE' ? 'bg-emerald-500/10 text-emerald-300 border-emerald-400/20' : b.decision === 'REJECT' ? 'bg-rose-500/10 text-rose-300 border-rose-400/20' : 'bg-amber-500/10 text-amber-300 border-amber-400/20'}`}>{b.decision}</span></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-slate-400"><div>تكلفة <strong className="text-slate-100">{b.measuredCostKwd.toFixed(3)}</strong></div><div>إنتاجية <strong className="text-slate-100">{b.yieldQuantity}</strong></div><div>زمن <strong className="text-slate-100">{b.prepTimeMinutes}د</strong></div><div>هدر <strong className="text-slate-100">{b.wastePercentage}%</strong></div></div><p className="text-slate-400 leading-6">{b.tastingResult}</p>{b.proposedChanges && <div className="flex gap-2 text-[11px] text-sky-300"><AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />{b.proposedChanges}</div>}</div>)}</div>
      </section>
