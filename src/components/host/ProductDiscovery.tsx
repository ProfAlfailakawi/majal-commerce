...
export const ProductDiscovery: React.FC = () => {
  ...
  if (!host) return <div className="glass-panel rounded-2xl p-6 text-center text-stone-500">لا توجد منشأة صالحة لهذا الحساب.</div>;
    ...
    <div className="space-y-6 text-stone-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-stone-950 border border-stone-800 rounded-xl pr-9 pl-3 py-2 text-xs text-stone-100 focus:outline-none focus:border-amber-500"
            />
          </div>
          ...
          <div className="flex items-center gap-2 bg-stone-950 p-2 rounded-xl border border-stone-800 text-xs text-stone-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {availableProducts.map(p => {
          ...
          return (
            <div key={p.id} className="bg-stone-900/90 rounded-2xl border border-stone-800 p-6 space-y-4 shadow-xl flex flex-col justify-between">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start justify-between gap-2 border-b border-stone-800 pb-3">
                  <div className="flex items-center gap-3">
                    <img src={p.mediaUrls[0]} alt={p.publicName} loading="lazy" decoding="async" className="w-14 h-14 rounded-xl object-cover ring-1 ring-stone-700" />
                    <div>
                      <h3 className="font-black text-stone-100 text-base">{p.publicName}</h3>
                      <span className="text-xs text-stone-400 block">بواسطة: <strong className="text-amber-400">{creator?.displayName}</strong></span>
                    </div>
                  </div>

                  {/* Match Score Badge */}
                  <div className="p-2.5 rounded-xl bg-amber-500/20 text-amber-300 border border-amber-500/30 text-center">
                    <span className="text-[10px] text-stone-400 block">نسبة المطابقة</span>
                    <span className="text-lg font-black font-mono">{matchCalc.overallScore}٪</span>
                  </div>
                </div>

                <p className="text-stone-300 text-xs leading-relaxed">{p.shortDescription}</p>
                ...
                <div className="bg-stone-950 p-3.5 rounded-xl border border-stone-800/80 space-y-2 text-xs">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center text-[10px] pt-1">
                    <div className="p-1.5 rounded bg-stone-900 border border-stone-800">
                      <span className="text-stone-500 block">توافق المعدات</span>
                      <span className="font-bold text-stone-200">{matchCalc.equipmentFit}٪</span>
                    </div>
                    <div className="p-1.5 rounded bg-stone-900 border border-stone-800">
                      <span className="text-stone-500 block">ملاءمة الهامش</span>
                      <span className="font-bold text-amber-400">{matchCalc.marginFit}٪</span>
                    </div>
                    <div className="p-1.5 rounded bg-stone-900 border border-stone-800">
                      <span className="text-stone-500 block">مطابقة الجمهور</span>
                      <span className="font-bold text-stone-200">{matchCalc.brandFit}٪</span>
                    </div>
                  </div>
                </div>
              ...
              {/* Actions Footer */}
              <div className="p-3 bg-stone-950 rounded-xl border border-stone-800 flex items-center justify-between text-xs mt-4">
                ...
                {hasApprovedGrant ? (
                  <button
                    ...
                    className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-stone-950 font-bold rounded-xl transition-colors flex items-center gap-1.5"
                  >
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <button
                      ...
                      className="px-3 py-2 bg-amber-500 hover:bg-amber-400 text-stone-950 font-black rounded-xl transition-colors flex items-center gap-1.5 shadow-md"
                    >
