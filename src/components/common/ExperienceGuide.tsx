    ...
    <div className="fixed inset-0 z-[90] bg-slate-950/80 backdrop-blur-md flex items-start justify-center p-4 pt-[12vh]" onMouseDown={event => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="command-title" className="w-full max-w-xl glass-panel rounded-[28px] border border-white/10 shadow-2xl overflow-hidden">
        <div className="flex items-center gap-3 p-4 border-b border-white/10">
          <Search className="w-5 h-5 text-[#e8c880]" />
      ...
      <section aria-label="التوجيه الذكي" className="relative z-20 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-5">
        <div className="glass-card rounded-2xl border border-white/10 px-4 py-3 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-xl">
          <div className="flex items-start sm:items-center gap-3 min-w-0">
            <span className="w-10 h-10 shrink-0 rounded-xl bg-[#c7a55b]/15 border border-[#e8c880]/20 grid place-items-center text-[#e8c880]"><Compass className="w-5 h-5" /></span>
            <div className="min-w-0">
              <span className="block text-[9px] font-black tracking-wide text-[#e8c880] uppercase">{nextMove.eyebrow}</span>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={() => onSurfaceChange(nextMove.surface)} className="px-4 py-2.5 rounded-xl bg-[#c7a55b] text-slate-950 text-xs font-black flex items-center gap-2 hover:bg-[#d5b76e]"><span>{nextMove.action}</span><ArrowLeft className="w-4 h-4" /></button>
          </div>
        </div>
      </section>

      {store.guardNotice && <div role="alert" aria-live="assertive" className="fixed bottom-5 right-5 z-[95] w-[min(420px,calc(100vw-2.5rem))] rounded-2xl bg-[#151b2b]/98 border border-rose-400/25 shadow-2xl p-4 flex items-start gap-3">
