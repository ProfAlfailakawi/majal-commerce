  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-stone-950/70 backdrop-blur-sm animate-in fade-in">
      <div className="absolute inset-y-0 left-0 max-w-full flex pl-0 sm:pl-10">
        <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="ai-drawer-title" className="w-screen max-w-md bg-stone-900 border-r border-stone-800 shadow-2xl flex flex-col text-stone-100">
          ...
          <div className="p-5 bg-stone-800/90 border-b border-stone-700 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 id="ai-drawer-title" className="font-bold text-sm text-stone-100">
            ...
            <button onClick={onClose} aria-label="إغلاق مساعد مجال" className="p-1.5 text-stone-400 hover:text-stone-100 rounded-lg hover:bg-stone-700">
              <X className="w-5 h-5" />
            </button>
          </div>

          {/* Mode Switcher */}
          <div className="p-3 bg-stone-950 border-b border-stone-800 flex gap-2 text-xs">
            <button
              onClick={() => setMode('POLISH')}
              className={`flex-1 py-2 px-3 rounded-lg font-bold flex items-center justify-center gap-1.5 transition-colors ${
                mode === 'POLISH' ? 'bg-amber-500 text-stone-950' : 'bg-stone-800 text-stone-300 hover:bg-stone-700'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg font-bold flex items-center justify-center gap-1.5 transition-colors ${
                mode === 'EXPLAIN_MATCH' ? 'bg-amber-500 text-stone-950' : 'bg-stone-800 text-stone-300 hover:bg-stone-700'
              }`}
            >
          ...
          <div className="p-5 flex-1 overflow-y-auto space-y-4 text-xs">
              ...
              <textarea
                ...
                placeholder={
                  mode === 'POLISH'
                    ...
                    : 'مثال: قرص عقيلي فاخر بالكريمة الفيلكاوية'
                }
                className="w-full bg-stone-950 border border-stone-800 rounded-xl p-3 text-stone-200 focus:outline-none focus:border-amber-500 resize-none"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-amber-500 hover:bg-amber-400 disabled:bg-stone-800 text-stone-950 font-bold rounded-xl transition-colors flex items-center justify-center gap-2 shadow-md"
            >
              {loading ? (
                <>
          ...
          {/* Footer note */}
          <div className="p-4 bg-stone-950 border-t border-stone-800 text-[10px] text-stone-500 text-center">
