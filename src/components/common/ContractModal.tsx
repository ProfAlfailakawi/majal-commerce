...
export const ContractModal: React.FC<ContractModalProps> = ({
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      if (!status.verified) {
        setSignError(`طلب هويتي لم يُعتمد بعد (الحالة: ${status.status}).`);
        return;
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/85 backdrop-blur-md animate-in fade-in">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="contract-modal-title" className="bg-stone-900 border border-stone-800 rounded-2xl max-w-3xl w-full overflow-hidden shadow-2xl text-stone-100 flex flex-col max-h-[92vh]">
        ...
        <div className="p-5 bg-stone-800/90 border-b border-stone-700 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 id="contract-modal-title" className="font-black text-lg text-stone-100">
          ...
          <button onClick={onClose} aria-label="إغلاق مسودة العقد" className="p-2 text-stone-400 hover:text-stone-100 rounded-lg hover:bg-stone-700">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Contract Legal Body */}
        <div className="p-6 overflow-y-auto space-y-6 text-xs leading-relaxed text-stone-300">
          ...
          <div className="p-4 bg-stone-950 rounded-xl border border-stone-800 space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-stone-500 block">الطرف الأول (المبدع وصاحب الحق):</span>
                <span className="font-bold text-stone-100 block">{contract.creatorLegalName}</span>
              </div>
              <div>
                <span className="text-stone-500 block">الطرف الثاني (المنشأة الحاضنة المرخّصة):</span>
                <span className="font-bold text-stone-100 block">{contract.hostCommercialName}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-stone-950 p-4 rounded-xl border border-stone-800 space-y-3">
            <h4 className="font-bold text-stone-200 text-sm">شروط الشراكة التجارية والمالية:</h4>
            
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="p-2.5 bg-stone-900 rounded-lg border border-stone-800">
                <span className="text-stone-500 text-[10px] block">سعر البيع المعتمد</span>
                <span className="font-bold text-amber-400 text-sm">{contract.terms.sellingPriceKwd.toFixed(3)} د.ك</span>
              </div>
              <div className="p-2.5 bg-stone-900 rounded-lg border border-stone-800">
                <span className="text-stone-500 text-[10px] block">نسبة حقوق المبدع Royalty</span>
                <span className="font-bold text-amber-400 text-sm">{contract.terms.creatorRoyaltyRatePercent}٪ من المبيعات</span>
              </div>
              <div className="p-2.5 bg-stone-900 rounded-lg border border-stone-800">
                <span className="text-stone-500 text-[10px] block">عمولة تشغيل المنصة</span>
                <span className="font-bold text-stone-300 text-sm">{contract.terms.platformFeePercent}٪</span>
              </div>
              <div className="p-2.5 bg-stone-900 rounded-lg border border-stone-800">
                <span className="text-stone-500 text-[10px] block">مدة العقد الحصري</span>
                <span className="font-bold text-stone-200 text-sm">{contract.terms.termMonths} شهراً</span>
              </div>
              <div className="p-2.5 bg-stone-900 rounded-lg border border-stone-800">
                <span className="text-stone-500 text-[10px] block">نطاق الحصرية</span>
                <span className="font-bold text-stone-200 text-sm">{contract.terms.exclusivityType === 'EXCLUSIVE' ? 'حصري لدولة الكويت' : 'غير حصري'}</span>
              </div>
              <div className="p-2.5 bg-stone-900 rounded-lg border border-stone-800">
                <span className="text-stone-500 text-[10px] block">الحد الأدنى للإنتاج</span>
                <span className="font-bold text-stone-200 text-sm">{contract.terms.minimumCommitmentUnits} قطعة</span>
              </div>
            </div>
          </div>

          {/* Legal Clauses */}
          <div className="space-y-3 p-4 bg-stone-950/60 rounded-xl border border-stone-800 text-[11px] text-stone-400 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            ...
            <div className={`p-4 rounded-xl border ${contract.creatorSignedAt ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-stone-950 border-stone-800 text-stone-400'}`}>
              ...
              {contract.creatorSignedAt ? (
                <div className="text-[11px]">
                  <p className="font-bold text-stone-100">{contract.creatorLegalName}</p>
                  <p className="text-stone-400 text-[10px]">تاريخ التوقيع: {new Date(contract.creatorSignedAt).toLocaleString('ar-KW')}</p>
                  <p className="text-stone-500 text-[9px] font-mono">Session Audit Ref: {contract.creatorSignerIp}</p>
                </div>
              ) : (
            ...
            {/* Host signature box */}
            <div className={`p-4 rounded-xl border ${contract.hostSignedAt ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-stone-950 border-stone-800 text-stone-400'}`}>
              <div className="flex items-center justify-between mb-1">
              ...
              {contract.hostSignedAt ? (
                <div className="text-[11px]">
                  <p className="font-bold text-stone-100">{contract.hostCommercialName}</p>
                  <p className="text-stone-400 text-[10px]">تاريخ التوقيع: {new Date(contract.hostSignedAt).toLocaleString('ar-KW')}</p>
                  <p className="text-stone-500 text-[9px] font-mono">Session Audit Ref: {contract.hostSignerIp}</p>
                </div>
              ) : (
                <span className="text-amber-400 font-semibold text-[11px]">في انتظار التوقيع...</span>
              )}
          ...
          {canCurrentUserSign && !hasUserSigned && contract.status !== 'FULLY_SIGNED' && (
            <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl space-y-3">
              ...
              <div>
                <label className="block text-stone-300 mb-1">الاسم القانوني الكامل للتوقيع:</label>
                <input
                  type="text"
                  value={signatureName}
              ...
              {!IS_DEMO_MODE && (
                <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-stone-950 border border-stone-800 rounded-lg p-2 text-stone-100 font-mono"
                  />
                  {paciRequestId && <p className="text-[10px] text-stone-400">طلب PACI: <span className="font-mono text-blue-300">{paciRequestId}</span> — {paciStatus || 'PENDING'}</p>}
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded bg-stone-800 border-stone-700 text-amber-500 focus:ring-amber-500"
                />
                <label htmlFor="agree_terms" className="text-stone-300 cursor-pointer">
              ...
              <button
                ...
                className="w-full py-2.5 bg-amber-500 hover:bg-amber-400 disabled:bg-stone-800 disabled:text-stone-600 text-stone-950 font-black rounded-xl transition-colors flex items-center justify-center gap-2"
              >
          ...
          {!canCurrentUserSign && contract.status !== 'FULLY_SIGNED' && (
            <div className="p-4 rounded-xl bg-sky-500/5 border border-sky-400/15 text-xs text-stone-400 leading-6">
