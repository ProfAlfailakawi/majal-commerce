  ...
  return (
    <section className="glass-panel rounded-3xl border border-white/10 p-5 md:p-6 space-y-5">
      ...
      <div className="grid xl:grid-cols-[.9fr_1.1fr] gap-5">
            ...
            <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/10 gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-slate-400 mt-1">المسؤول: {task.owner} — {task.detail}</div>
                </div>
              </div>
              <span className={`text-xs font-bold shrink-0 ${task.done ? 'text-emerald-300' : 'text-amber-300'}`}>{task.done ? 'مكتمل' : 'قيد التنفيذ'}</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-xs text-slate-400">{decisions.length} سجلات بشرية محفوظة</span>
          </div>
      ...
      <div className="rounded-2xl p-4 bg-indigo-500/5 border border-indigo-400/15 space-y-3">
        ...
        <p className="text-xs text-slate-400 leading-6">{aiNarrative?.narrative || copilot.summary}</p>
        <div className="grid md:grid-cols-2 gap-3">
          <div className="space-y-2">
            <div className="text-xs font-bold text-slate-300">الخيارات</div>
            {copilot.optionSummaries.map((option, idx) => (
              <div key={idx} className="text-xs text-slate-400 rounded-lg px-3 py-2 bg-white/5 border border-white/10">{option}</div>
            ))}
          </div>
          <div className="space-y-2">
            <div className="text-xs font-bold text-slate-300">مخاطر ونقاط انتباه</div>
            {copilot.risks.map((risk, idx) => (
              <div key={idx} className={`text-xs rounded-lg px-3 py-2 border ${risk.level === 'HIGH' ? 'bg-rose-500/10 border-rose-400/20 text-rose-200' : 'bg-amber-500/10 border-amber-400/20 text-amber-200'}`}>{risk.text}</div>
            ))}
            {(aiNarrative?.watchouts || []).map((watch, idx) => (
              <div key={`ai-${idx}`} className="text-xs rounded-lg px-3 py-2 bg-white/5 border border-white/10 text-slate-400">{watch}</div>
            ))}
          </div>
        </div>
        <div className="flex flex-wrap gap-2 pt-2 border-t border-white/5">
          {copilot.blockedActions.map((guard, idx) => (
            <span key={idx} className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-slate-950/50 border border-white/10 text-slate-400">
