...
export const AdminAuditLogs: React.FC = () => {
  ...
  return (
    <div className="bg-stone-900/90 rounded-2xl border border-stone-800 p-6 space-y-4 text-stone-100 text-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-800 pb-3">
        <div>
          <h3 className="font-bold text-base text-stone-100 flex items-center gap-2">
            ...
            <span>سجل التدقيق والأمان الرقمي Audit Log</span>
          </h3>
        ...
        <table className="w-full text-right">
          <thead>
            <tr className="border-b border-stone-800 text-stone-400">
              ...
              <th className="py-2.5 px-3 font-semibold">نوع الحدث Action</th>
              <th className="py-2.5 px-3 font-semibold">نوع الكيان Entity</th>
          ...
          <tbody className="divide-y divide-stone-800/60 font-mono">
            {filteredLogs.map(log => (
              <tr key={log.id} className="hover:bg-stone-800/40 transition-colors">
                <td className="py-2.5 px-3 text-stone-400">{new Date(log.timestamp).toLocaleString('ar-KW')}</td>
                <td className="py-2.5 px-3">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30">
                    {log.action}
                  </span>
                </td>
                <td className="py-2.5 px-3 text-stone-300 font-bold">{log.entityType}</td>
                <td className="py-2.5 px-3 text-stone-200 font-sans">{log.details}</td>
                <td className="py-2.5 px-3 text-stone-500 text-[10px]">{log.ipAddress}</td>
              </tr>
            ))}
          </tbody>
        </table>
