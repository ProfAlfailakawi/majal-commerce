...
export interface DealDecision {
  ...
  authorRole: UserRole;
  text: string;
  category: 'DECISION' | 'NOTE' | 'RISK' | 'MILESTONE';
  createdAt: string;
}
...
export interface Order {
  ...
  hostBusinessId: string;
  branchId?: string;
  acquisitionSource?: 'CREATOR' | 'HOST' | 'MAJAL' | 'UNKNOWN';
  customerName: string;
  ...
  hostNetKwd: number;
  status: 'PENDING_PAYMENT' | 'COMPLETED' | 'REFUNDED';
  createdAt: string;
}
...
export interface Accrual {
  ...
  settlementStatus: 'ACCRUED' | 'SETTLEMENT_ELIGIBLE' | 'SETTLEMENT_LOCKED' | 'PAID';
...
// Kuwait F&B Commercial Integrations
export type PaymentGatewayProvider = 'KNET' | 'TAP_PAYMENTS' | 'MY_FATOORAH' | 'HESABE';

export interface PaymentTransaction {
  id: string;
  orderId?: string;
  collaborationId?: string;
  type: 'ORDER_PAYMENT' | 'LAB_DEPOSIT' | 'CREATOR_PAYOUT' | 'ESCROW_HOLD';
...
export interface PaciMobileIdVerification {
  id: string;
  userId: string;
  civilId: string;
  documentType: 'CONTRACT' | 'NDA' | 'COMPLIANCE_SIGN';
  documentId: string;
  documentTitle: string;
  challengeCode: string;
  status: 'PENDING_AUTHENTICATION' | 'VERIFIED' | 'REJECTED' | 'EXPIRED';
  signatureHash?: string;
  verifiedAt?: string;
  expiryTime: string;
}

export type PosSystemProvider = 'FOODICS' | 'DELIVERECT' | 'URBAN_PIPER' | 'SYRVE' | 'CUSTOM_WEBHOOK';
...
export interface PosIntegrationConfig {
  ...
  royaltyCalculationRule: 'GROSS_SALES' | 'NET_OF_DISCOUNTS';
