...
- `src/lib/runtime.ts`: وضع العرض **لا يُفعَّل تلقائياً أبداً**. الزائر وحده يفعّله بضغط
  "ادخل العرض" في تبويبه (علم في `sessionStorage` يختفي بإغلاق التبويب)، ولا يلمس أي حساب حقيقي.
  لإلغاء إمكانية العرض كلياً في حزمة الإنتاج: ابنِ بـ`VITE_DISABLE_DEMO_MODE=true`.
  الصفحة العامة لا تعرض بعد الآن مدخل عرض لطبقتي الأدمن/السوبر أدمن.
